.PHONY: help build up up-dmr up-cloud down logs logs-app logs-ollama logs-qdrant restart clean pull-models status test-backend test-frontend security-check compose-validate compose-smoke quality-gate shell-app

help:
	@echo "Personal AI Docker Compose Commands"
	@echo "===================================="
	@echo "make build          - Build all images"
	@echo "make up             - Start all services (Ollama container)"
	@echo "make up-dmr         - Start all services (Docker Model Runner, macOS only)"
	@echo "make up-cloud       - Start all services (cloud LLM inference via .env.cloud)"
...
up:
	docker compose up -d
	@echo ""
	@echo "Services started (Ollama container mode)"
...
up-dmr:
	docker compose -f docker-compose.yml -f docker-compose.dmr.yml up -d
...
up-cloud:
	...
	@echo ""
	@echo "Services started (cloud inference mode)"
	@echo "  App:        http://localhost:8000"
	@echo "  (Chat → cloud provider, Embeddings → local Ollama)"
	@echo ""
	@echo "Services started (Docker Model Runner mode)"
	@echo "  App:        http://localhost:8000"
	@echo "  Qdrant:     http://localhost:6333"
	@echo "  Prometheus: http://localhost:9090"
	@echo "  Grafana:    http://localhost:3000"
	@echo "  (Ollama container disabled — models served by DMR)"
