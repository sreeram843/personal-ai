...
cleanup() {
  docker compose --profile local -f "$COMPOSE_FILE" down --remove-orphans >/dev/null 2>&1 || true
}

trap cleanup EXIT

printf '\n[%s] Starting smoke services (qdrant + redis)\n' "$(date '+%H:%M:%S')"
docker compose --profile local -f "$COMPOSE_FILE" up -d qdrant redis >/dev/null
